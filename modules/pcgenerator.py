...
def calibrate():
    ...
    return '<?xml version="1.0" encoding="UTF-8" standalone="no"?>{}'.format(
        diagram.tostring()
    )
    ...
    def __init__(
        ...
        if self.card_rows > 200 or self.card_stitches > machine_config["stitches"]:
            raise ValueError(
                ...
                "Are you sure you uploaded the right text file?".format(
                    machine_config["stitches"]
                )
            )

        self.horz_repeat = int(horz_repeat)
        self.vert_repeat = int(vert_repeat)
        self.is_blank = is_blank

        self.card_height = (self.pattern_hole_yoffset * 2) + (
            ((self.card_rows * self.vert_repeat) - 1) * self.row_height
        )
...
class PCGenerator:
    def __init__(
        ...
        logger.debug(f"machine_id: {machine_id}")
    ...
    def generate(self):
        ...
        objects = []
        if (
            self.layout.overlapping_rows
            and self.layout.overlapping_row_xoffset
            and self.layout.overlapping_row_yoffset
        ):
            self.draw_overlapped_lines(diagram, objects)
        if self.layout.clip_hole_diameter and self.layout.clip_hole_xoffset:
            print("hey1")
            self.draw_clip_holes(diagram, objects)
        if self.layout.tractor_hole_diameter and self.layout.tractor_hole_xoffset:
            print("hey2")

            self.draw_tractor_holes(diagram, objects)

        if self.layout.is_blank:
            self.layout.pattern_hole_diameter = 0.5
        print("hey3")
        ...
        sorted_objects = sorted(
            objects, key=lambda x: (float(x.attribs["cy"]), float(x.attribs["cx"]))
        )
    ...
    def draw_pattern(self, diagram, lines, objects):
        print("drw pattern", self.layout.vert_repeat)
            ...
            for rows in range(self.layout.card_rows):
                ...
                for stitch_repeat in range(self.layout.horz_repeat):
                    for stitches in range(self.layout.card_stitches):
                        try:
                            if (lines[rows]).decode("utf-8")[stitches].upper() == "X":
                                objects.append(
                                    diagram.circle(
    ...
    def draw_side_holes(self, diagram, objects, xoffset, yoffset, diameter):
        ...
        while yoffset <= self.layout.card_height:
            ...
            if (
                yoffset >= self.layout.card_height
                and not self.layout.half_hole_at_bottom
            ):
                break
    ...
    def get_card_shape(self):
        ...
        g = (
            self.layout.card_width - self.layout.corner_offset,
            self.layout.card_height - 20,
        )
        h = (
            self.layout.card_width - self.layout.corner_offset,
            self.layout.card_height - 1,
        )
